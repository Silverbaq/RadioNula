...
import com.radionula.radionula.resources.nula_logo_info
...
import com.radionula.radionula.features.comments.CommentsScreen
import com.radionula.radionula.core.ui.components.NoConnectionOverlay
import com.radionula.radionula.core.ui.components.NulaTopBar
import com.radionula.radionula.features.favorites.FavoritesScreen
import com.radionula.radionula.features.player.PlayerScreen
import com.radionula.radionula.core.ui.theme.BackgroundBlue
...
fun NulaApp(
    ...
    // Assume connected until the monitor says otherwise, so the overlay does
    // not flash over the player on every cold start.
    val connected by connectivityMonitor.isOnline.collectAsStateWithLifecycle(true)
        ...
        Column(Modifier.fillMaxSize().background(BackgroundBlue)) {
            ...
            Box(Modifier.fillMaxSize()) {
                NavHost(navController, startDestination = ROUTE_PLAYER) {
                    composable(ROUTE_PLAYER) { PlayerRoute(snackbarHostState = snackbarHostState) }
                    composable(ROUTE_FAVORITES) {
                        FavoritesRoute(snackbarHostState = snackbarHostState)
                    }
...
private fun PlayerRoute(
    ...
    LaunchedEffect(viewModel) {
        viewModel.favoriteAdded.collect { title ->
            snackbarHostState.showSnackbar("Added $title to favorites")
        }
    }
...
private fun FavoritesRoute(
    ...
    webSearch: WebSearch = koinInject(),
) {
    ...
    FavoritesScreen(
        ...
        onRemove = { track ->
            ...
            scope.launch { snackbarHostState.showSnackbar("Removed ${track.artist}") }
        },
        onSearch = { track -> webSearch.search("${track.artist} ${track.title}") },
    )
}
    ...
    ModalDrawerSheet(
        ...
        Column(Modifier.fillMaxSize()) {
            Box(
                ...
                Image(
                    painter = painterResource(Res.drawable.nula_logo_info),
